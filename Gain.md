...
Set Gunbot to buy when the price reaches a fixed percentage below the lowest EMA at that moment, and sell when the price is a defined percent higher than the price it bought for.

You can optionally use RSI as confirmation to only buy or sell when both a specific gain and RSI level is reached.
...
| `BUY_LEVEL`          | 0.6           | *Values: numerical – represent a percentage.* This sets the target for buying with gain as buying strategy, at a percentage below the lowest EMA at that moment. |
| `GAIN`               | 0.6           | *Values: numerical – represents a percentage.*This sets the target for selling with Gain as selling strategy. The bot will sell when the set percentage above last buying price is reached. |
...
| `HIGH_BB`            | 49            | *Values: numerical – represents a percentage.* This sets the target for selling with bb as selling strategy. The bot will sell when the price hits a point 40% from HIGH_BB. This is only used for using `DOUBLE_UP `in gain strategy. |
| `LOW_BB`             | 49            | *Values: numerical – represents a percentage.* This sets the target for buying with bb as buying strategy. The bot will buy when the price hits a point 40% from LOW_BB. This is only used for using `DOUBLE_UP` in gain strategy. |
